...
import { PlusCircle, FileText, MessageSquareText, ChevronLeft, Upload, Send, HelpCircle, BookOpen } from 'lucide-react'; // Using lucide-react for icons
...
function App() {
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setCurrentView(selectedRole === 'old_member' ? 'qa_selection' : 'new_member_qa');
  ...
  const startProjectQaSession = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/transfer/project-qa/start-session/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const respondToProjectQa = async () => {
    ...
    try {
      ...
      setProjectQaHistory(prev => [...prev, { role: 'human', content: projectQaAnswer }]);

      const response = await fetch(`${API_BASE_URL}/transfer/project-qa/respond/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.next_question) {
        setProjectQaHistory(prev => [...prev, { role: 'ai', content: data.next_question }]);
      } else {
  ...
  const handleUploadDocument = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/transfer/document/?project_id=${selectedProject.id}`, {
        method: 'POST',
        body: formData,
      });
      if (!response.ok) {
      ...
      showMessage(`Document '${data.file_name}' uploaded successfully! Now generate questions.`);
    } catch (error) {
  ...
  const handleGenerateQuestions = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/transfer/document-qa/generate-questions/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ project_id: selectedProject.id, document_id: uploadedDocument.id }),
      });
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || `HTTP error! status: ${response.status}`);
  ...
  const fetchNextDocQuestion = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/transfer/document-qa/get-next-question/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ project_id: selectedProject.id, document_id: uploadedDocument.id }),
      });
  ...
  const answerDocQuestion = async () => {
    if (!docQaAnswer.trim() || !currentDocQuestion || currentDocQuestion.is_complete) {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/transfer/document-qa/answer-question/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          project_id: selectedProject.id,
      ...
      if (data.is_complete) {
        ...
        showMessage(data.message || 'Answer recorded. Here is the next question.');
      }
    } catch (error) {
  ...
  const askNewMemberQuery = async () => {
    ...
    try {
      ...
      setNewMemberChatHistory(prev => [...prev, { role: 'human', content: newMemberQuery }]);
      ...
      const data = await response.json();
      setNewMemberChatHistory(prev => [...prev, { role: 'ai', content: data.answer, sources: data.source_documents }]);
  ...
  const renderRoleSelection = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md text-center">
        <h2 className="text-3xl font-bold text-gray-800 mb-6">Welcome to Knowledge Relay</h2>
        <p className="text-gray-600 mb-8">Please select your role to get started:</p>
        <div className="flex flex-col space-y-4">
          <button
            onClick={() => { setSelectedRole('old_member'); setCurrentView('project_selection'); }}
          ...
          <button
            onClick={() => { setSelectedRole('new_member'); setCurrentView('project_selection'); }}
  ...
  const renderProjectSelection = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-2xl">
        ...
        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Select or Create Project</h2>

        {/* Create Project Section */}
        <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
          <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center"><PlusCircle size={20} className="mr-2" /> Create New Project</h3>
          <input
            type="text"
        ...
        <div className="p-6 border border-gray-200 rounded-lg bg-gray-50">
          <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center"><FileText size={20} className="mr-2" /> Select Existing Project</h3>
          {loading ? (
            <p className="text-gray-500 text-center">Loading projects...</p>
          ) : projects.length === 0 ? (
            <p className="text-gray-500 text-center">No projects available. Create one above!</p>
          ) : (
            <select
              onChange={(e) => {
  ...
  const renderQaSelection = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md text-center">
        ...
        <p className="text-gray-600 mb-8">Choose how you want to contribute knowledge:</p>
        <div className="flex flex-col space-y-4">
          <button
          ...
          <button
            onClick={() => { setQaType('document_qa'); setCurrentView('document_qa'); }}
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-3xl flex flex-col h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 border border-gray-200 rounded-lg mb-4 bg-gray-50 flex flex-col space-y-4">
          {projectQaHistory.length === 0 && !loading && (
            <p className="text-gray-500 text-center py-8">Start by answering the first question...</p>
          )}
          {projectQaHistory.map((msg, index) => (
            <div
              key={index}
              className={`p-3 rounded-lg shadow-sm ${
                msg.role === 'human' ? 'bg-blue-100 self-end text-right' : 'bg-gray-200 self-start text-left'
              }`}
              style={{ maxWidth: '80%' }}
            >
              <p className="font-semibold text-sm mb-1">{msg.role === 'human' ? 'You' : 'AI'}</p>
              <p className="text-gray-800">{msg.content}</p>
            </div>
          ))}
          {loading && (
            <div className="text-center text-gray-500">Loading next question...</div>
          )}
        </div>
        ...
        {projectQaSession && !projectQaSession.is_complete ? (
          <div className="flex items-center space-x-3">
            <input
              ...
              onKeyPress={(e) => { if (e.key === 'Enter') respondToProjectQa(); }}
          ...
          <p className="text-center text-gray-600 font-semibold mt-4">
            {projectQaSession?.message || "Session completed or no questions available."}
          </p>
        )}
        {errorMessage && (
          <p className="text-red-500 mt-4 text-sm text-center">{errorMessage}</p>
  ...
  const renderDocumentQa = () => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-3xl">
        ...
        <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
          <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center"><Upload size={20} className="mr-2" /> Upload Document</h3>
          <input
            type="file"
          ...
          {uploadedDocument && (
            <p className="text-green-600 mt-3 text-center">Uploaded: {uploadedDocument.name}</p>
          )}
        </div>
        ...
        {uploadedDocument && (
          <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
            <h3 className="text-xl font-semibold text-gray-700 mb-4 flex items-center"><PlusCircle size={20} className="mr-2" /> Generate Questions</h3>
            <p className="text-gray-600 mb-4">Click below to generate questions from the uploaded document. These will then be available for you to answer.</p>
            <button
        ...
        {docQaHistory.length > 0 && (
          <div className="mt-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
            <h3 className="text-xl font-semibold text-gray-700 mb-4">Answered Questions:</h3>
              ...
              {docQaHistory.map((entry, index) => (
                <div key={index} className="p-3 rounded-lg bg-white shadow-sm">
                  ...
                  {entry.source_context && (
                    <p className="text-xs text-gray-500 mt-1">Source: {entry.source_context.substring(0, 100)}...</p>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-3xl flex flex-col h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 border border-gray-200 rounded-lg mb-4 bg-gray-50 flex flex-col space-y-4">
          {newMemberChatHistory.length === 0 && !loading && (
            <p className="text-gray-500 text-center py-8">Ask a question to get started...</p>
          )}
                ...
                <div className="mt-2 text-xs text-gray-600 border-t border-gray-300 pt-2">
                  ...
                  {msg.sources.map((source, sIdx) => (
                    <div key={sIdx} className="mb-1">
                      <p className="font-medium">{source.file_name || 'Static Q&A'}</p>
                      {source.question && <p className="ml-2">Q: {source.question}</p>}
                      <p className="ml-2 italic">Context: {source.context.substring(0, 150)}...</p>
                      {source.page_number && <p className="ml-2">Page: {source.page_number}</p>}
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="flex items-center space-x-3">
          <input
            ...
            onKeyPress={(e) => { if (e.key === 'Enter') askNewMemberQuery(); }}
            className="flex-1 p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
            disabled={loading}
          />
          <button
